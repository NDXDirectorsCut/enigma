...
public class EnigmaCamera : NetworkBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        {
            transform.position = Vector3.Lerp(transform.position,hit.point+transform.forward*buffer,1);           //hit.point+transform.forward*buffer;//Vector3.MoveTowards(transform.position,hit.point + transform.forward*buffer,60*Time.deltaTime);
